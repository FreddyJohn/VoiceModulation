package com.adams.voicemodulation;
...
import com.adams.voicemodulation.audio.Export;
import com.adams.voicemodulation.audio.RecordLogic;
import com.adams.voicemodulation.controls.ModulateControls;
import com.adams.voicemodulation.database.AppDatabase;
import com.adams.voicemodulation.database.ProjectDao;
import com.adams.voicemodulation.database.project.AudioData;
import com.adams.voicemodulation.database.project.Paths;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private String[] phaser_titles;
    private String[] phaser_quantities;
    private final int[] phaser_progress = new int[]{1, 10, 0};
    private final int[] phaser_maxes = new int[]{20, 20, 10};
    private String[] flanger_titles;
    private String[] flanger_quantities;
    ...
    public static GraphLogic graph;
    ...
    public static Structure bitmapPieceTable;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        phaser_titles = new String[]{getString(R.string.freq), getString(R.string.amplitude), getString(R.string.title_theta)};
        phaser_quantities = new String[]{getString(R.string.hz), getString(R.string.amp), getString(R.string.theta)};
        ...
        phaser_scales = new double[]{nyquist, .05, .1 * Math.PI};
        ...
        ModulateControls flanger_view = new ModulateControls(this, flanger_titles, flanger_maxes, flanger_scales,
                flanger_quantities, flanger,
                getString(R.string.flanger_sine_title), flanger_progress, play_button, projectInfo);
        scrollView.addView(flanger_view);
    }

    public static void setDisplayStream(int buffsize, String file, boolean state, int range, int n) {
        display.setEncoding(range);
        display.setGraphState(state, buffsize, file, n);
    }
    public static int getLength() {
        audioPieceTable = audioPieceTable.getMostRecent();
        return audioPieceTable.byte_length;
    }
    public static Pair<Integer, Integer> getAudioSelectionPoints() {
        audioPieceTable = audioPieceTable.getMostRecent();
    ...
    public void onClick(View v) {
        ...
        if (hasPermissions(this, PERMISSIONS)) {
            ...
            if (vId == R.id.undo) {
                ...
                int[] backwards_maxes = new int[]{20};
                ModulateControls backwards_view = new ModulateControls(this, backwards_titles, backwards_maxes, new double[]{.05},
                        new String[]{getString(R.string.volume)}, backwards,
                ...
                ModulateControls echo_view = new ModulateControls(this, echo_titles, echo_maxes, new double[]{.02},
                        new String[]{getString(R.string.s)}, echo,
                ...
                ModulateControls phaser_view = new ModulateControls(this, phaser_titles, phaser_maxes, phaser_scales,
                        phaser_quantities, phaser,
                ...
                scrollView.removeAllViews();
                Modulation.effect PhaserTriangle = new Modulation.phaserTriangle();
                ModulateControls phaser_triangle_view = new ModulateControls(this, phaser_titles, phaser_maxes, phaser_scales,
                        phaser_quantities, PhaserTriangle,
                ...
                ModulateControls phaser_square_view = new ModulateControls(this, phaser_titles, phaser_maxes, phaser_scales,
                        phaser_quantities, phaserSquare,
                ...
                ModulateControls phaser_saw_view = new ModulateControls(this, phaser_titles, phaser_maxes, phaser_scales,
                        phaser_quantities, phaserSaw,
                ...
                ModulateControls flanger_triangle_view = new ModulateControls(this, flanger_titles, flanger_maxes, flanger_scales,
                        flanger_quantities, flangerTriangle,
                ...
                ModulateControls flanger_square_view = new ModulateControls(this, flanger_titles, flanger_maxes, flanger_scales,
                        flanger_quantities, flangerSquare,
                ...
                ModulateControls amp_view = new ModulateControls(this, new String[]{getString(R.string.volume)}, new int[]{20}, new double[]{.05},
                        new String[]{" "}, amp,
                ...
                //String freq = audioData.sample_rate + getString(R.string.hz);
                //frequency.setText(freq);
                //memory.setText(Convert.memory(length));
                //time.setText(Convert.time(max));
                ...
                new Thread(() -> {
                    ...
                    try {
                        points = getAudioSelectionPoints();
                ...
                alert.setCancelable(false).setPositiveButton(R.string.save, (dialogBox, id) -> {
                    //newProject.project_name = dialog.getText().toString();
    ...
    private void addListeners(SubMenu savedProject, CharSequence title) {
            ...
            DialogInterface.OnClickListener dialogClickListener = (dialog, which) -> {
                switch (which){
                    case DialogInterface.BUTTON_POSITIVE:
                        Project deleteProject = userDao.getProjectFromName((String)title);
                        if(deleteProject.paths.uniqueDir.equals(newProject.paths.uniqueDir)){
                            graph = findViewById(R.id.display);
                            initializeProjectData();
                            initializeProjectStructures();
                            graph.drawable = null;
                        ...
                        File parentDir = new File(this.getExternalFilesDir("Projects")+
                                "/"+deleteProject.paths.uniqueDir);
        ...
        savedProject.add(getString(R.string.rename)).setOnMenuItemClickListener(item -> {
            ...
            alert.setCancelable(false).setPositiveButton(R.string.rename, (dialogBox, id) -> {
                Project renameProject = userDao.getProjectFromName((String)title);
                String newName = dialog.getText().toString();
                System.out.println(dialog.getText().toString());
                userDao.updateProjectName(newName, renameProject);
            }).setNegativeButton(getString(R.string.cancel), (dialogBox, id) ->{
                dialogBox.cancel();
            });
            AlertDialog alertDialogAndroid = alert.create();
